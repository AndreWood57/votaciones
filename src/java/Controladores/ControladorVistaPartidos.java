 ...
 * @author andre
 */
public class ControladorVistaPartidos extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            try {
                ...
                ArrayList<Partido> partidos = new Operaciones().getPartidos(conexion);
                if (partidos.size() > 0) {
                    HttpSession session = request.getSession(true);
                    session.setAttribute("Partidos", partidos);
                    response.sendRedirect("Views/VistaMenuVotanteRealizarVoto.jsp");
                }
